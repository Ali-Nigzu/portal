...
def test_default_mapping_includes_demodata(monkeypatch):
    ...
    assert mapping["demo_data.clientA"] == "timestamp"
    assert mapping["demo_data.client1"] == "timestamp"
    assert mapping["clientA"] == "timestamp"
...
def test_env_override_ignored_for_locked_orgs(monkeypatch, caplog):
    monkeypatch.setenv(
        ...
        json.dumps({"demo_data.clientA": "event_ts", "other": "custom_ts"}),
    )
    monkeypatch.setattr(org_config, "ORG_EVENT_TIMESTAMP_COLUMNS", {})
