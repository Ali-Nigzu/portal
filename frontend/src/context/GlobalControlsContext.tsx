...
const GlobalControlsContext = createContext<
  GlobalControlsContextValue | undefined
>(undefined);
...
export const GlobalControlsProvider: React.FC<{
  children: React.ReactNode;
}> = ({ children }) => {
  const initialState = useMemo(getInitialState, []);
  const [rangePreset, setRangePresetState] = useState<RangePreset>(
    initialState.rangePreset,
  );
  const [customRange, setCustomRangeState] = useState<CustomRange | undefined>(
    initialState.customRange,
  );
  const [granularity, setGranularityState] = useState<GranularityOption>(
  ...
  const [compare, setCompareState] = useState<CompareOption>(
  ...
  const stepRange = useCallback(
    (direction: "forward" | "backward") => {
      ...
      const nextIndex = Math.min(
        Math.max(currentIndex + directionValue, 0),
        presets.length - 1,
      );
