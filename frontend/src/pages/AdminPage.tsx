...
const AdminPage: React.FC<AdminPageProps> = ({ credentials }) => {
  const [activeTab, setActiveTab] = useState<"users" | "alarms" | "devices">(
    "users",
  );
  ...
  const loadAdminData = React.useCallback(async () => {
    try {
      ...
      setAlert({
        message: `Failed to load admin data: ${err instanceof Error ? err.message : "Unknown error"}`,
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  const loadDataSources = React.useCallback(
    async (clientId: string) => {
      try {
        const auth = btoa(`${credentials.username}:${credentials.password}`);
        const response = await fetch(
  ...
  const handleViewDashboard = async (username: string) => {
    try {
      const auth = btoa(`${credentials.username}:${credentials.password}`);
      const response = await fetch(
        `${API_BASE_URL}/api/admin/create-view-token`,
        {
          method: "POST",
          headers: {
            Authorization: `Basic ${auth}`,
  ...
  const handleEditAlarm = async (e: React.FormEvent) => {
    ...
    try {
      const auth = btoa(`${credentials.username}:${credentials.password}`);
      const response = await fetch(
        `${API_BASE_URL}/api/admin/alarm-logs/${editingAlarm.id}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Basic ${auth}`,
  ...
  const handleEditDevice = async (e: React.FormEvent) => {
    ...
    try {
      const auth = btoa(`${credentials.username}:${credentials.password}`);
      const response = await fetch(
        `${API_BASE_URL}/api/admin/device-list/${editingDevice.id}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Basic ${auth}`,
  ...
  const handleAddDataSource = async (e: React.FormEvent) => {
    ...
    try {
      const auth = btoa(`${credentials.username}:${credentials.password}`);
      const response = await fetch(
        `${API_BASE_URL}/api/admin/data-sources/${selectedClient}`,
        {
          method: "POST",
          headers: {
            Authorization: `Basic ${auth}`,
  ...
  const handleEditDataSource = async (e: React.FormEvent) => {
    ...
    try {
      const auth = btoa(`${credentials.username}:${credentials.password}`);
      const response = await fetch(
        `${API_BASE_URL}/api/admin/data-sources/${selectedClient}/${editingDataSource.id}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Basic ${auth}`,
  ...
  const handleDeleteDataSource = async (sourceId: string) => {
    ...
    try {
      const auth = btoa(`${credentials.username}:${credentials.password}`);
      const response = await fetch(
        `${API_BASE_URL}/api/admin/data-sources/${selectedClient}/${sourceId}`,
        { method: "DELETE", headers: { Authorization: `Basic ${auth}` } },
      );
      const data = await response.json();
      if (data.success) {
  ...
  const handleSetActiveDataSource = async (sourceId: string) => {
    try {
      const auth = btoa(`${credentials.username}:${credentials.password}`);
      const response = await fetch(
        `${API_BASE_URL}/api/admin/data-sources/${selectedClient}/${sourceId}/set-active`,
                  ...
                  <tbody>
                    ...
                    {users.map((user) => (
                      <tr key={user.username}>
                        ...
                        <td>
                          <span
                            className={`vrm-status ${user.role === "admin" ? "vrm-status-warning" : "vrm-status-online"}`}
                          >
                            {" "}
                            {user.role}{" "}
                          </span>
                          ...
                          <div style={{ display: "flex", gap: "8px" }}>
                            ...
                            {user.role === "client" && (
                              <button
                                ...
                                onClick={() =>
                                  handleViewDashboard(user.username)
                                }
                              >
                                {" "}
                                View Dashboard{" "}
                    ...
                    {newUser.role === "client" && (
                      <div style={{ marginBottom: "16px" }}>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewUser({ ...newUser, csv_url: e.target.value })
                          }
                          style={{
                            width: "100%",
                            padding: "8px",
                  ...
                  <form onSubmit={handleAddAlarm}>
                    <div style={{ marginBottom: "16px" }}>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewAlarm({ ...newAlarm, instance: e.target.value })
                        }
                        style={{
                          width: "100%",
                          padding: "8px",
                    ...
                    <div style={{ marginBottom: "16px" }}>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewAlarm({ ...newAlarm, device: e.target.value })
                        }
                        style={{
                          width: "100%",
                          padding: "8px",
                    ...
                    <div style={{ marginBottom: "16px" }}>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEditingAlarm({
                            ...editingAlarm,
                            alarmClearedAfter: e.target.value,
                          })
                        }
                        style={{
                          width: "100%",
      ...
      {activeTab === "devices" && (
        <div>
          ...
          <div className="vrm-card">
            <div className="vrm-card-header">
              <h3 className="vrm-card-title">
                Device List for {selectedClient}
              </h3>
              <div className="vrm-card-actions">
                <button
                  className="vrm-btn vrm-btn-sm"
