...
const AnalyticsPage: React.FC<AnalyticsPageProps> = ({ credentials }) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (genderTimeFilter.option !== 'alltime') {
        const now = new Date();
        let startDate: Date;
        let endDate: Date = now;

        switch (genderTimeFilter.option) {
          case 'last24h':
          ...
          case 'last30days':
            ...
            break;
          case 'custom':
            if (genderTimeFilter.startDate && genderTimeFilter.endDate) {
              startDate = new Date(genderTimeFilter.startDate);
              endDate = new Date(genderTimeFilter.endDate + 'T23:59:59');
            } else {
              startDate = now;
            }
            break;
  ...
  }, [credentials.username, credentials.password, genderTimeFilter]);
