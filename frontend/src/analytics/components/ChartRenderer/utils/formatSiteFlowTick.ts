...
const formatMonth = (date: Date, withYear = false): string =>
  date.toLocaleDateString("en-US", { month: "short", year: withYear ? "numeric" : undefined });
...
export const formatSiteFlowTick = (
  ...
  switch (timeframe) {
    ...
    case "last_month":
      return bucket === "WEEK" ? formatWeekLabel(parsed) : String(parsed.getDate());
    case "last_quarter":
...
export const formatSiteFlowTooltip = (
  timeframe: string | undefined,
  bucket: string | undefined,
  label: string,
): string => {
